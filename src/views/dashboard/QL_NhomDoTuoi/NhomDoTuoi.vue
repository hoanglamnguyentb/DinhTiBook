<template>
  ...
  <a-row>
    <a-col :span="24">
      <a-card title="Danh sách nhóm độ tuổi">
        <template #extra>
        ...
        <div>
          <a-row style="margin-bottom: 20px" :gutter="24">
            <a-col :span="12">
              <a-input
                v-model:value="formSearch.TenNhomFilter"
...
import ModalAddEditNhomDoTuoi from './ModalAddEditNhomDoTuoi.vue';
export default {
  page: {
  ...
  data() {
    return {
      ...
      items: [
        ...
        {
          text: "Quản lý nhóm độ tuổi",
          active: true,
        },
      ],
      tableColumns: [
        { title: "STT", width: 100, key: "STT" },
        { title: "Mã nhóm", dataIndex: "maNhomDoTuoi", width:300},
        { title: "Tên nhóm", dataIndex: "tenNhom", width:450 },
  ...
  methods: {
    ...
    deletedata(event) {
      Swal.fire({
        ...
        if (result.value) {
          try {
            //xóa trên database
            await APIService.delete("/NhomDoTuoi/delete/" + event.id);
    ...
    SearchData() {
      ...
      var searchParam = {
        ...
        TenNhomFilter: this.formSearch.TenNhomFilter,
    ...
    async loadData(pageIndex, pageSize, params) {
      ...
      var result = await APIService.get("/NhomDoTuoi?" + urlQuery);
      this.lstNhomDoTuoi = result.data.data.items;
    ...
    openModalAdd() {
      this.$refs.modalAddEditNhomDoTuoi.showModal();
    },
    openModalEdit(id){
      this.$refs.modalAddEditNhomDoTuoi.showModal(id);
    }
  },
  mounted() {
    this.loadData(this.optionPage.pageIndex, this.optionPage.pageSize);
