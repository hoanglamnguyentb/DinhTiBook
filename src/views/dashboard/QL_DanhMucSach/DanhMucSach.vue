      ...
      <a-card title="Danh sách danh mục">
        ...
        <div>
          <a-row style="margin-bottom: 20px" :gutter="24">
            <a-col :span="12">
              <a-input
                v-model:value="formSearch.CategoryNameFilter"
...
import ModalAddEditDanhMucSach from './ModalAddEditDanhMucSach.vue';
export default {
  page: {
  ...
  data() {
    return {
      ...
      tableColumns: [
        ...
        { title: "Mã danh mục", dataIndex: "maCategory", width:300},
        { title: "Tên danh mục", dataIndex: "categoryName", width:450 },
  ...
  methods: {
    ...
    deletedata(event) {
      Swal.fire({
        ...
        if (result.value) {
          try {
            //xóa trên database
            await APIService.delete("/DanhMuc/delete/" + event.id);
    ...
    SearchData() {
      ...
      var searchParam = {
        ...
        CategoryNameFilter: this.formSearch.CategoryNameFilter,
    ...
    async loadData(pageIndex, pageSize, params) {
      ...
      var result = await APIService.get("/DanhMuc/GetDataByPage?" + urlQuery);
