<template>
  ...
  <a-row>
    <a-col :span="24">
      <a-card title="Danh sách slide tác giả">
        <template #extra>
...
export default {
  page: {
    title: "Quản lý slide tác giả",
    meta: [
      {
        name: "Danh sách slide tác giả",
  ...
  methods: {
    ...
    async loadData(pageIndex, pageSize, params) {
      ...
      var result = await APIService.get("/TTSlide?TypeFilter=TACGIA&" + urlQuery);
