<template>
  <div>
    <a-modal
      v-model:open="open"
      title="Thêm slide khách hàng"
      @ok="handleOk"
      @cancel="closeModal"
    >
      <a-form ref="formRef" :model="slide"  layout="vertical"  :rules="rules">
        <a-form-item ref="thongTin1" label="Tên khách hàng" name="thongTin1">
        ...
        <a-form-item ref="thongTin3" label="Mô tả" name="thongTin3">
          <a-textarea
                v-model:value="slide.thongTin3"
        ...
        <a-form-item label="Ảnh khách hàng">
              <FileManager
                    ref="childRef"
     :parentId="newId"
                    :SoLanHienThi="1"
...
export default {
  components:{
    ...
    return {
      ...
      rules:{
        thongTin1:{
          required: true, message: 'Thông tin hàng không được để trống', trigger: 'change'
        },
        thongTin2:{
    ...
    handleOk() {
      this.$refs.formRef.validate().then(async () => {
        const data ={
          ...
          thongTin3: this.slide.thongTin3,
          type: "KHACHHANG"
        }
